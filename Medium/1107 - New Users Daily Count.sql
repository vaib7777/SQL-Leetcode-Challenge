...
WITH login_count AS
(
SELECT *, ROW_NUMBER() OVER (PARTITION BY user_id ORDER BY activity_date) log_time
FROM Traffic
WHERE activity = 'login'
)
SELECT activity_date AS login_date, COUNT(*) user_count
FROM login_count
WHERE DATEDIFF('2019-06-30', activity_date) <= 90 AND log_time = 1 GROUP BY 1 ORDER BY 1
