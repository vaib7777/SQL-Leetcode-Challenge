...
with cte as(select sum(quantity)/count(distinct product_id) as maxx from ordersdetails group by order_id order by maxx desc limit 1)

select order_id from ordersdetails group by order_id having max(quantity)>(select * from cte)